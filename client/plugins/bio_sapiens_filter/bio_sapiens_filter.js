...
(function ($) {
	var myDiv;
	var id = 'bs_filter';
	var ols_widget_url = 'plugins/bio_sapiens_filter/resources/js/ui.olscheckboxtree.js';
	...
	var init = function(){
		var desc = 'Filter annotations by terms in the Biosapiens Ontology';
		var plugin_info = {id: id, description:desc, type:'Filter', filter: filter, name:'Bio Sapiens Filter', save_state:save_state};
	...
	var on_got_annotations = function(e, params){
		...
		for(var i=0, l=features.length; i<l; i++){
			...
			if(typeof type_id != 'undefined'){
				if(type_id !='' && type_id.indexOf('BS:') != -1){
	...
	var on_search_done = function(){
		DASTY.log('Painting biosapiens tree');
		myDiv.empty();
		...
		$(function(){
				$(myDiv).olscheckboxtree({
					...
					collapsed: true,
        			selected: true,
					collapse_image: 'plugins/bio_sapiens_filter/resources/images/open.png',
					expand_image: 'plugins/bio_sapiens_filter/resources/images/close.png',  
	...
	var filter = function(segment_annotations){
		DASTY.log('Filtering by biosapiens ontology');
		if (myDiv) {
			if (flag) {
