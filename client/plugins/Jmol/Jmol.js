...
(function ($) {
	 ...
	 * Jmol Variables
	 */
	var jmoljarpath = "plugins/Jmol/resources/"; // where Jmol.jar is to be found on a server
    var jmollocaljarpath = "./plugins/Jmol/resources/"; // where Jmol.jar is to be found locally
    var jmoljarfile = "JmolApplet.jar";  // "JmolApplet.jar" is everything in one file 
 	var dataDir = "./plugins/Jmol/";                // where the model AND .spt files are to be found.
	var _jmol;
	var targetSuf = -1;
	var conf;
	
	var jmol_js_path = 'plugins/Jmol/resources/Jmol.js';  // where Jmol.js is to be found on a server
	
	/**
	 *  Plugin variables
	 */
	var use_proxy = true;
	var pdb_files_url = 'http://www.ebi.ac.uk/pdbe-srv/view/files/';
	var alignment_url = 'http://www.rcsb.org/pdb/rest/das/pdb_uniprot_mapping/alignment?query=';
	var alignments = [];
	var protein_id;
	...
	var init = function(){
		var desc = 'PDB 3D protein structure';
		conf = DASTY.getConfigurationData(id) || {};
		plugin_info = {id: id, description:desc, type:'Display', name:'3D Structure', save_state:save_state};
	...
	var on_search_started = function(e, params){
		...
		protein_id = params.searchId;
		$.ajax({
			type: "GET",
			url: DASTY.getProxyURL()+'?url='+alignment_url+protein_id,
			dataType: "text",
			success: parse_response,
			error: function(){DASTY.log('DASTY could not get the alignment')}
		});
	}
	...
	var parse_response = function(xml){
		...
		$(xml).find('block').each(function(){
			...
			var segment0 = create_node(chd[0]);
			var segment1 = create_node(chd[1]);
			alignments[segment0.intObjectId] = [segment0, segment1] || [];
			i++;
		});
		alignments.length = i;
		initJmol();
	...
	var create_node = function(segment){
		var obj = {
			intObjectId : $(segment).attr('intObjectId') || '', 
			start : parseInt($(segment).attr('start')) || '', 
			end : parseInt($(segment).attr('end')) || ''
		}
		return obj;
	}
	
	var initJmol = function(){
		if (alignments.length > 0) {
			var sel_arr = []; 
			sel_arr[0] = '<select id="' + id + '_select">';
			var i = 1;
			for (var al in alignments) {
				sel_arr[i] = '<option value=' + al + '>' + al + '</option>';
				i++;
			}
			sel_arr[i+1] = '</select>';
			
			var table = '<table width="100%"><tr><td>'+sel_arr.join('')+'</td></tr>'+
						'<tr><td><div id="Jmol0" class="JmolDiv" style="position:relative; height: 400px; z-index: 1"></div></td></tr>'+
						'</table>';
			myDiv.html(table);
			...
			if(typeof conf.selectedPDB != 'undefined'){
				$('#' + id + '_select').val(conf.selectedPDB);
			}
			
			on_selection_change();
		}else{
			myDiv.append('<span style="color:gray; font-size:9px;">Dasty could not find PDBs associated to '+protein_id+'</span>')
			DASTY.log('Dasty could not find PDBs associated to '+protein_id);
		}
	}
	
	var on_selection_change = function(){
		var url = pdb_files_url;
		if(use_proxy){
			url = DASTY.getProxyURL()+'?url='+url;
		}
		
		var selected_pdb = $('#'+id+'_select').val().toLowerCase();
		...
		document.getElementById("Jmol0").innerHTML = jmolApplet('100%','defaultDirectory = "'+url+'";load '+selected_pdb+'.pdb; select all; cartoon on; wireframe off; spacefill off; color chain;');
		targetSuf ++;
	}
	
	var select_PDBResidues = function(start, end){      
		var scr='selectionHalos ON; select '+start+'-'+end;
		jmolScriptWait(scr, targetSuf);
	}
	
	
	var on_selection = function(e,params){
		if (alignments.length > 0) {
			var start = parseInt(params.start);
			var end = parseInt(params.end);
			var align = alignments[$('#' + id + '_select').val()];
			var uni = align[1];
			if ((start >= uni.start && start <= uni.end) || (end >= uni.start && end <= uni.end) || (start < uni.start && end > uni.end)) {
				...
				var scr='selectionHalos OFF';
				jmolScriptWait(scr,targetSuf);
			}
		}
	}
	DASTY.registerListener('sequence_selection', on_selection);
	DASTY.registerListener('selected_feature', on_selection);
	
	/* Save current state */
	var save_state= function(){
		var selected_pdb = $('#'+id+'_select').val();
		return '"selectedPDB":"'+selected_pdb+'"';
	}
