...
(function ($) {
	var myDiv;
	var id = 'eco_filter';
	var ols_widget_url = 'plugins/evidence_code_filter/resources/js/ui.olscheckboxtree.js';
	...
	var init = function(){
		var desc = 'Filter annotations by terms in the Evidence Codes Ontology';
		var plugin_info = {id: id, description:desc, type:'Filter', filter: filter, name:'Evidence Codes Filter', save_state:save_state};
	...
	var on_got_annotations = function(e, params){
		...
		for(var i=0, l=features.length; i<l; i++){
			var feat = features[i];
			var type_id ='';
			if (typeof feat.method != 'undefined') {
				type_id = feat.method.cvId || '';
			}
			if(type_id=='' || typeof type_id=='undefined'){
				type_id = feat.type.category || '';
				type_id = type_id.substring(type_id.indexOf('(')+1, type_id.indexOf(')'));
			}
			if(typeof type_id != 'undefined'){
				if(type_id !='' && type_id.indexOf('ECO:') != -1){
	...
	var on_search_done = function(){
		DASTY.log('Painting evidence codes tree');
		myDiv.empty();
		...
		$(function(){
				$(myDiv).olscheckboxtree({
					...
					collapsed: true,
        			selected: true,
					collapse_image: 'plugins/evidence_code_filter/resources/images/open.png',
					expand_image: 'plugins/evidence_code_filter/resources/images/close.png',   
					ids:categories, 
	...
	var filter = function(segment_annotations){
		DASTY.log('Filtering by evidence code');
		if (myDiv) {
			if (flag) {
