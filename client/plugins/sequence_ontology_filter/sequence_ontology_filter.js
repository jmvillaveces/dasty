...
(function ($) {
	var myDiv;
	var id = 'so_filter';
	var ols_widget_url = 'plugins/sequence_ontology_filter/resources/js/ui.olscheckboxtree.js';
	...
	var init = function(){
		var desc = 'Filter annotations by terms in the Sequence Ontology';
		var  plugin_info = {id: id, description:desc, type:'Filter', filter: filter, name:'Sequence Ontology Filter', save_state:save_state, get_ontology_terms:get_ontology_terms};
	...
	var on_got_annotations = function(e, params){
		...
		for(var i=0, l=features.length; i<l; i++){
			...
			if(typeof type_id != 'undefined'){
				if(type_id !='' && type_id.indexOf('SO:') != -1){
	...
	var on_search_done = function(){
		DASTY.log('Painting sequence ontology tree');
		myDiv.empty();
		...
		var unselected_ids = config.unselected_ids || ['SO:0000104'];//polypeptide
		...
		$(function(){
				$(myDiv).olscheckboxtree({
					...
					collapsed: true,
        			selected: true,
					collapse_image: 'plugins/sequence_ontology_filter/resources/images/open.png',
					expand_image: 'plugins/sequence_ontology_filter/resources/images/close.png',  
	...
	/* Get Ontology Terms */
	var get_ontology_terms = function(){
		var x = $(myDiv).olscheckboxtree('get_all');// || [];
		return x;
	}
	...
	var filter = function(segment_annotations){
		DASTY.log('Filtering by Sequence Ontology');
		if (myDiv) {
			...
			for (var i = 0, l = segment_annotations.length; i < l; i++) {
				...
				if(features){
					for (var j = 0, k = features.length; j < k; j++) {
						...
						var type_cv_id;
						if (typeof feat.type != 'undefined') {
							type_cv_id = feat.type.cvId || feat.type.id;
						}
						
						if (typeof type_cv_id != 'undefined') {
							for (var m = 0, n = checked.length; m < n; m++) {
								var chk = checked[m].id || checked[m];
								if (chk == type_cv_id) {
									add = false;
									break;
								}
